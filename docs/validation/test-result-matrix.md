...
| qtest (device) | `tests/qtest/ethos-u-test, imx93-*-test` | MMIO-level device behaviour, golden compares | PASS (13) |
...
| ISI + MIPI-CSI + MT9M114 / OV5640 | A | PASS (2) | Real V4L2 frames to /dev/video0; v4l2-compliance 48/48 ioctl + 55/55 streaming | Camera chip-IDs are the real datasheet values AND driver-validated: ov5640 0x5640 (ov5640_check_chip_id errors otherwise), mt9m114 0x2481. The PCA9538 camera expander (0x70 on LPI2C8) resets to its TI-datasheet POR (config 0xFF = all inputs, output 0xFF), not memset-0's all-outputs; imx93-pca9538-test drives a real LPI2C8 master transaction to read it back, mutation-proven |
| SAI3 + WM8962 codec | A | PASS (2) | Real PCM playback ASSERTED on the captured samples: run.sh always captures to a wav (driver=wav opens a file, never a device - simultaneously the mute and the evidence) and check_wav.py requires peak == 8000, ~1s of signal, and a 436 Hz tone, so a datapath that scaled, truncated or mis-paced the stream fails. Mutation-verified: halving the amplitude and dropping half the samples each fail, and each is missed by the other's gate. TCSR/RCSR FIFO request/warning flags gated on TE/RE - a disabled transmitter reports no FIFO request, matching the RM | VERID reports the RM's version (3.2), not an invented 3.3; the FEATURE word is deliberately 0 where silicon says 2 (bit 1 = TSTMP_EN, which fsl_sai gates real code on) because the timestamp engine is not modelled - under-reporting a capability is the only direction safe both here and on the board. SAI access width pinned to 16-bit for eDMA S16 writes. The SAI is a bit-clock SLAVE (TCR2.BCD_MSTR clear), so the frame rate is not derivable from its registers - they are byte-identical at 48 kHz and 16 kHz. The WM8962 decodes R27 ADDITIONAL_CONTROL_3 and drives the rate to the SAI's codec-rate input, as the board does; the SAI opens its voice and paces its FIFO from it. Mutation-verified: cut the wire and a 16 kHz stream is clocked out 3x too fast (tone 145 Hz -> 436 Hz, a third of the duration). Codec decoder taken from the i.MX 95 node |
| MICFIL (PDM mic) / XCVR | B | — | Driver bring-up; ALSA cards register | MICFIL VERID/PARAM report the RM's values (0x020F_0000 / 0x010B_0154) with only the HWVAD capability masked out, because the voice-activity detector is not modelled - under-reporting a capability is the safe direction. PARAM used to advertise an 8-deep FIFO, a number nobody had: the RM says 32, the driver's imx93 soc_data says 32, and this model's array holds 64. A build assertion now decodes the advertised depth from PARAM the way the guest does and requires the array to be at least that deep |
...
| FlexSPI | B | PASS (1) | qtest (NOR bring-up + reset values / earned DLL lock) | AHBRXBUFnCR0 reset per-index to the RM's 0x800n_0020 (MSTRID = buffer index; a blanket seed mis-IDs every buffer). STS2 resets to 0x0100_0100 with the four DLL-lock bits CLEAR; the lock is EARNED when firmware writes DLLxCR[DLLEN] (nxp-fspi polls STS2 for it and would time out against memset-0), never seeded. Mutation-proven three ways incl. seed-locked-at-reset -> fails, enforcing earned-not-seeded. Latent in the stock dtb (FlexSPI status=disabled); the qtest is the reader |
