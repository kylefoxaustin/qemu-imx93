...
| **PCA9451A PMIC reset values** | The 4 BUCK4/5/6 + LDO1 voltage-select resets in `hw/i2c/imx93_i2c_regdev.c` were bring-up scaffolding at DT-range selectors, not silicon voltages — BUCK4 (EVK 3.3V SD supply) read back ~1.625V | **FIXED — bit-exact to datasheet OTP:** now `0x6C/0x30/0x14/0xC2` per the PCA9451A datasheet (rev 2.1, `93_docs/PCA9451A-datasheet.pdf`, Table 17 + bit-field tables). First **derived** (fact-sheet voltages + mainline `pca9450` encoding), then **confirmed** against the datasheet: 3 bucks matched to the bit; the datasheet corrected LDO1CTRL to `0xC2` (`ENMODE=11` Always-ON, which the derivation left `00`). Boot-verified: guest regulator sysfs reads 3.3/1.8/1.1/1.8V, all `state=enabled`, 0 failures. Other rails (BUCK1-3, LDO2-5) still at memset-0; full OTP-faithful seed is a possible follow-up now the datasheet is in-repo |
