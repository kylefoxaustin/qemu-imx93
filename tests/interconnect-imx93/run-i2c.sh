...
boot "$RECV_IRD" "-chardev socket,id=i2cl,path=$SOCK,server=on,wait=off" "$RLOG"; SPID=$!; reap_track $SPID
sleep 2
echo "== booting i.MX 93 SENDER (i2c-link socket connect) =="
boot "$SEND_IRD" "-chardev socket,id=i2cl,path=$SOCK,server=off,reconnect-ms=1000" "$SLOG"; CPID=$!; reap_track $CPID
