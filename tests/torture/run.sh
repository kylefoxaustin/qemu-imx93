...
# Wayland desktop + an animating client, the full audio surface (wm8962
# play+capture, SPDIF, MICFIL), CPU stress on both A55s, SD storage churn and
# network - then checks that none of them wedged and the display kept scanning
# out frames. The Ethos-U NPU is intentionally excluded (its M33 boot wedges the
# desktop guest - see README.md "Known issues"). See README.md for the approach.
#
...
rm -rf "$OVERLAY"; mkdir -p "$OVERLAY"/{usr/lib,opt,progress}
