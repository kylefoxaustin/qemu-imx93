 ...
 *
 * enet-lab3: the i.MX 93 (Linux/A55) node for the fleet raw-L2 segment.
 *
 ...
 * (both bare-metal M-core) and the sibling of 95emulator's tests/enet-lab3.
 * It broadcasts a 64-byte L2 frame carrying OUR ethertype over the i.MX 93's
 * FEC (eth0) - the same real datapath that already passes byte-exact b2b in
 * tests/interconnect-imx93/run-eth.sh - onto a QEMU socket segment (rt1180's
 * NETC switch fabric, or the mcast hub), and watches inbound frames for the
 * OTHER nodes' ethertypes.
 *
 * PROVENANCE: the WIRE BODY below (magic/self-ethertype/seq/incarnation/fill,
 * bytes 14..63) is the fleet's RATIFIED v2 spec - demonstrated-interoperable
 * across mcx/imx91/imx95/rt1180 - and is reproduced VERBATIM; "Do not improve
 * any of these numbers" is load-bearing, so only this header is 93-specific.
 * Built from THIS repo per the fleet rule that every node ships from its own
 * tree (a peer's copy is that peer's rehearsal stand-in, never the real node).
 *
 *   usage: enet-lab3 <ifname> <my_ethertype> <peer_ethertype> [more_peers...]
 *   e.g.   enet-lab3 eth0 0x88B9 0x88B5 0x88B6 0x88B7    (imx93 watching the segment)
 *
 * ---------------------------------------------------------------------------
